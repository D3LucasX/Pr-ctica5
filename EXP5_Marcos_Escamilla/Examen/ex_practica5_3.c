...
int main(){
	int cantidad_productos, i, reabastecer = 0, agregar_cant = 0, encontrado = 0, retirar = 0, retirar_cant = 0;//Añadimos la variable retirar, para ingresar las unidades que el usuario quiera retirar
...
//Preguntamos al usuario si quiere retirar
	printf("Si desea retirar algún producto,pulse 1, si no pulse 2:\n");
	scanf(" %d", &retirar);
	if (retirar != 1){
		printf("Vale\n");
	}while(retirar == 1){//Aqui hacemos un bucle para que siempre retire los productos que el usuario quiere cuando haya suficientes

	printf("¿Que producto quiere retirar?");//Preguntamos el producto
	scanf(" %[^\n]", nombre_producto);
	for(i = 0; i < cantidad_productos; i++){//Bucle para recorrer todos los productos
		if(strcmp(nombre_producto, Informatica[i].nombre) == 0){ //Nos devuelve 0 si son exactas
	printf("Que cantidad quiere retirar?\n");
	scanf(" %d", &retirar_cant);
		Informatica[i].cantidad_stock -= retirar_cant;//Restamos la cantidad que queremos 
		printf("Producto actualizado, nueva cantidad de %d\n",Informatica[i].cantidad_stock);
